package com.kalbenutritionals.kalcaremobieTesting.ViewModel;

/**
 * Created by Robert on 28/02/2018.
 */

public class VMCustomers {
    String txtContactID;
    String txtAlamat;
    String txtNama;
    String txtKabKot;
    String txtProv;
    String txtjenisAlm;
    String txtListMed;
    String txtKec;
    String txtKel;
    String txtNHDSiteID;
    String txtProvID;
    String txtKABKotID;
    String txtKecID;
    String txtKodePos;

    public String getTxtContactID() {
        return txtContactID;
    }

    public void setTxtContactID(String txtContactID) {
        this.txtContactID = txtContactID;
    }

    public String getTxtAlamat() {
        return txtAlamat;
    }

    public void setTxtAlamat(String txtAlamat) {
        this.txtAlamat = txtAlamat;
    }

    public String getTxtNama() {
        return txtNama;
    }

    public void setTxtNama(String txtNama) {
        this.txtNama = txtNama;
    }

    public String getTxtKabKot() {
        return txtKabKot;
    }

    public void setTxtKabKot(String txtKabKot) {
        this.txtKabKot = txtKabKot;
    }

    public String getTxtProv() {
        return txtProv;
    }

    public void setTxtProv(String txtProv) {
        this.txtProv = txtProv;
    }

    public String getTxtjenisAlm() {
        return txtjenisAlm;
    }

    public void setTxtjenisAlm(String txtjenisAlm) {
        this.txtjenisAlm = txtjenisAlm;
    }

    public String getTxtListMed() {
        return txtListMed;
    }

    public void setTxtListMed(String txtListMed) {
        this.txtListMed = txtListMed;
    }

    public String getTxtKec() {
        return txtKec;
    }

    public void setTxtKec(String txtKec) {
        this.txtKec = txtKec;
    }

    public String getTxtKel() {
        return txtKel;
    }

    public void setTxtKel(String txtKel) {
        this.txtKel = txtKel;
    }

    public String getTxtNHDSiteID() {
        return txtNHDSiteID;
    }

    public void setTxtNHDSiteID(String txtNHDSiteID) {
        this.txtNHDSiteID = txtNHDSiteID;
    }

    public String getTxtProvID() {
        return txtProvID;
    }

    public void setTxtProvID(String txtProvID) {
        this.txtProvID = txtProvID;
    }

    public String getTxtKABKotID() {
        return txtKABKotID;
    }

    public void setTxtKABKotID(String txtKABKotID) {
        this.txtKABKotID = txtKABKotID;
    }

    public String getTxtKecID() {
        return txtKecID;
    }

    public void setTxtKecID(String txtKecID) {
        this.txtKecID = txtKecID;
    }

    public String getTxtKodePos() {
        return txtKodePos;
    }

    public void setTxtKodePos(String txtKodePos) {
        this.txtKodePos = txtKodePos;
    }
}
